    ...
    <el-form-item v-for="(item,index) in from[input].formConfig.formItems" :key="`index-${index}`" :label="item.label" :prop="item.prop" :style="{width:(from[input].formConfig.inline?'auto':parseFloat((item.span/24)*100,10)+'%'),display:'inline-block'}">
      ...
      <template v-if="!!item.child&&!!item.child.length">
        <template v-for="(item2,index2) in item.child">
          ...
          <el-form-item v-if="!!item2.prop" :prop="item2.prop" :key="`index2-${index2}`" :style="{display:'inline-block',marginBottom:'0px'}">
            <dynamic-form-el :elParentData="item" :elData="item2" :input="input"></dynamic-form-el>
          </el-form-item>
          <dynamic-form-el v-else :elParentData="item" :elData="item2" :input="input" :key="`index2-${index2}`"></dynamic-form-el>
          
        </template>
      </template>
      <template v-else>
        <!-- 适配各种输入组件 -->
        <dynamic-form-el :elParentData="item" :elData="item" :input="input"></dynamic-form-el>
...
<script>
import dynamicFormEl from './dynamicFormEl.vue';
